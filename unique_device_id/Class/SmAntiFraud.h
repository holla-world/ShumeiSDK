...
@interface SmOption : NSObject {
    ...
    NSString *_contactUrl;        // 上传短通通数据接口
    NSString *_confUrl;
...
@property(readwrite) NSString *contactUrl;
@property(readwrite) NSString *confUrl;
@property(readwrite) NSString *trackerUrl;
 ...
 *
 *  异步模式: 返回空字符串。
 *  同步模式: 成功：返回联系数据。  失败：返回空字符串。
 */
- (NSString*) getContact:(int) mode;
-(NSString*) getContactSyn;
